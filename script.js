...
animateCards();
cards.forEach(card => { card.addEventListener("mouseleave", () => { card.dataset.tx = 0; card.dataset.ty = 0; }); });
...
class Particle {
  constructor(x, y) { this.x = x; this.y = y; this.size = Math.random() * 3 + 1; this.speedX = Math.random() * 2 - 1; this.speedY = Math.random() * 2 - 1; this.color = `rgba(139,92,246,${Math.random() * 0.5 + 0.2})`; }
...
window.addEventListener('mousemove', e => { particles.push(new Particle(e.x+Math.random()*20-10,e.y+Math.random()*20-10)); if(particles.length>particleCount) particles.shift(); });
...
// ===== Fetch Current Day from day.json =====
const clockDisplay = document.getElementById("school-day");
fetch("/day.json?_=" + Date.now())
  .then(res => res.json())
  .then(data => { clockDisplay.textContent = data.currentDay ? data.currentDay + " Day" : "Day not found"; })
  .catch(err => { console.error(err); clockDisplay.textContent="Loading Day..."; });

// ===== Auto redirect mobile =====
if (window.innerWidth <= 768) window.location.href = "https://semester-1-schedule-mobile.neocities.org";